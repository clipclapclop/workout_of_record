...
Each set displays the fields required by its movement, such as reps, weight, distance, or time. Weight is recorded in pounds (`lbs`) and distance in miles (`mi`). Planned values appear as defaults. Confirming a set saves its completed values immediately. Clearing a required value prevents confirmation, and unchecking a completed set removes its confirmed values from the completed workout while leaving the editable values on screen.
